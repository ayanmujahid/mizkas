...
class ShopController extends Controller
{
    ...
    {
        $variation = Variation::with('variationBelongsToProduct', 'variationBelongsToColor', 'variationBelongsToSize')->latest()->get();
        $groupedVariations = $variation->groupBy(function ($item) {
            return $item->product_id . '-' . $item->color_id;
        });

        // Merge grouped variations
        $mergedVariations = $groupedVariations->map(function ($group) {
            // Assuming you want to merge size information as an example
            $mergedSizes = $group->pluck('variationBelongsToSize')->unique();

            // Combine other necessary attributes as required
            $mergedAttributes = [
                'id' => $group->first()->id,
                'product' => $group->first()->variationBelongsToProduct,
                'color' => $group->first()->variationBelongsToColor,
                'sizes' => $mergedSizes,
                'designer_price' => $group->first()->designer_price,
                'our_price' => $group->first()->our_price,
                'stock' => $group->first()->stock,
                'img_path' => $group->first()->img_path,
                'back_image' => $group->first()->back_image,
                // Add other attributes you want to merge here
            ];

            return $mergedAttributes;
        });

        // Convert to collection or array (if needed)
        $mergedVariationsCollection = $mergedVariations->values();
        $variations = $mergedVariationsCollection->toArray();
        // dd($mergedVariationsArray);
        return view('admin.variation-management.list')->with('title', 'Variation Management')->with('variation_menu', true)->with(compact('variations'));
    }
    ...
    public function add_variation()
    {
        ...
        return view('admin.variation-management.add')->with('title', 'Add Variation')->with('variation_menu', true)->with(compact('products', 'size', 'color'));
    }
    ...
    public function create_variation(Request $request)
    {
        ...
        $request->validate([
            ...
            'designer_price' => 'required|numeric',
            'our_price' => 'required|numeric',
        ]);
        if (request()->hasFile('main_image')) {
            $main_image = request()->file('main_image')->store('Uploads/Variation/' . rand() . rand(10, 100), 'public');
        }
        if (request()->hasFile('back_image')) {
            $back_image = request()->file('back_image')->store('Uploads/Variation/'  . rand() . rand(10, 100), 'public');
        }
        foreach ($request->size_id as $size) {
            $checkVariation  = Variation::where('product_id', $request['product_id'])->where('size_id', $size)->where('color_id', $request['color_id'])->first();
            if ($checkVariation == null) {
                $variation = Variation::create([
                    'product_id' => $request['product_id'],
                    'size_id' => $size,
                    'color_id' => $request['color_id'],
                    'stock' => $request['stock'],
                    'designer_price' => $request['designer_price'],
                    'our_price' => $request['our_price'],
                ]);
                if (request()->hasFile('main_image')) {
                    $maiimage = Variation::where('id', $variation->id)->update(
                        [
                            'img_path' => $main_image,
                        ]
                    );
                ...
                if (request()->hasFile('back_image')) {
                    $image2 = Variation::where('id', $variation->id)->update(
                        [
    ...
    {
        $variation = Variation::where('id', $id)->with('variationBelongsToProduct', 'variationBelongsToColor', 'variationBelongsToSize')->first();
        $variations = Variation::where('product_id', $variation->product_id)->where('color_id', $variation->color_id)->get();
        $products = Products::where('is_active', 1)->latest()->get();
        $size = Size::where('is_active', 1)->latest()->get();
        $color = Color::where('is_active', 1)->latest()->get();
        $variationSizes = [];
        foreach ($variations as $variation) {
            $variationSizes[] =  $variation->size_id;
        }

        $id = $id;

        return view('admin.variation-management.edit')->with('title', 'Edit Variation')->with('variation_menu', true)->with(compact('variation', 'variations', 'variationSizes', 'products', 'size', 'color', 'id'));
    }
    ...
    public function savevariation(Request $request)
    {
        ...
        if (request()->hasFile('main_image')) {
            ...
            $main_image = request()->file('main_image')->store('Uploads/Variation/' . $request->id . rand() . rand(10, 100), 'public');
        }
        if (request()->hasFile('back_image')) {
            $validator = Validator::make($request->all(), [
                'back_image' => 'required|mimes:jpeg,jpg,png,gif,webp',
            ]);

            $back_image = request()->file('back_image')->store('Uploads/Variation/' . rand() . rand(10, 100), 'public');
        }
        $curvariation = Variation::where('id', $request->id)->first();
        $variationIds = [];
        if ($request->size_id != null) {
            foreach ($request->size_id as $size) {

                $checkVariation  = Variation::where('product_id', $curvariation->product_id)->where('size_id', $size)->where('color_id', $curvariation->color_id)->first();
                if ($checkVariation != null) {
                    $variationIds[] = $checkVariation->id;
                    $variation = Variation::where('id', $checkVariation->id)->update([
                        'product_id' => $request['product_id'],
                        'size_id' => $size,
                        'color_id' => $request['color_id'],
                        'stock' => $request['stock'],
                        'designer_price' => $request['designer_price'],
                        'our_price' => $request['our_price'],
                        'img_path' => $curvariation->img_path,
                        'back_image' => $curvariation->back_image,
                    ]);
                    if (request()->hasFile('main_image')) {
                        $maiimage = Variation::where('id', $checkVariation->id)->update(
                            [
                                'img_path' => $main_image,
                            ]
                        );
                    ...
                    if (request()->hasFile('back_image')) {
                        $image2 = Variation::where('id', $checkVariation->id)->update(
                            [
                                'back_image' => $back_image,
                            ]
                        );
        ...
        $deleteRemainingVariation = Variation::where('product_id', $curvariation->product_id)->where('color_id', $curvariation->color_id)->whereNotIn('id', $variationIds)->delete();
