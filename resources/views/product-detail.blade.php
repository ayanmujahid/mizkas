  ...
  <div class="swiper mySwiperThumbs mt-3 d-none d-md-block">
    ...
    ><a href="javascript:;" class="size-btn 
                        ...
                        <div class="product-price">
                            <h3 class="title7">
                            <span id="product-price-holder">Price USD {{$product->price}}</span>    </h3>
                            <span id="product-price-holder" style="font-size: 15px;text-decoration: line-through;margin-top: 0px;">Old Price USD {{$product->old_price}}</span>    </h3>
...
.sizes li.active {
    background-color: green; /* Change background when active */
...
.sizes a.size-btn {
    ...
    width: 100%;
    height: 100%;
    line-height: 23px; /* Make the text vertically centered */
...
.sizes li.active a {
    color: white; /* Make the active text white */
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function attachToActive() {
    ...
    if (typeof activeDestroyFn === 'function') {
      try { activeDestroyFn(); } catch(e){ /* ignore */ }
    ...
    if (!img.complete) {
      img.addEventListener('load', function onLoad() {
        ...
        activeDestroyFn = createMagnifierForImage(img, { zoom: 2, size: 140 });
      });
    } else {
      activeDestroyFn = createMagnifierForImage(img, { zoom: 2, size: 140 });
    }
  ...
  document.querySelectorAll('.mySwiperThumbs .swiper-slide img').forEach(function(t){
    t.addEventListener('click', function(){ setTimeout(attachToActive, 50); });
  });

  // optional: reattach on window resize
  window.addEventListener('resize', function(){ setTimeout(attachToActive, 100); });
});
</script>
