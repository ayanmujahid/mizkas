                ...
                <form>
                    <table class="tf-table-page-cart">
                        <thead>
                            ...
                            @php
                            $product = App\Models\Products::find($val['product_id']);
                            $total += $val['sub_total'];
                            @endphp
                        </thead>
                        ...
                        <tbody>
                            <tr class="tf-cart-item file-delete" data-rowid="{{ $val['cart_id'] }}">
                                ...
                                <td class="tf-cart-item_product">
                                    <a href="{{ route('product-detail', $product->slug) }}" class="img-box">
                                        <img src="{{ isset($val['custom']) && $val['custom'] == 1 ? $val['image'] : asset($product->img_path) }}"
                                            alt="product">
                                    </a>

                                    <div class="cart-info">
                                        <a href="{{ route('product-detail', $product->slug) }}" class="cart-title link">
                                            {{ isset($val['custom']) && $val['custom'] == 1 ? 'Customized Product' : $product->title }}
                                        </a>

                                        <a href="{{ route('remove-cart', $val['cart_id']) }}"
                                            class="cart-delete"
                                            data-rowid="{{ $val['cart_id'] }}">
                                            <i class='bx bxs-trash'></i> Remove Item
                                        </a>
                                    </div>
                                </td>
                                ...
                                <td class="tf-cart-item_quantity" cart-data-title="Quantity">
                                    <div class="cart-quantity cart-quantity-inputs">
                                        <div class="wg-quantity">
                                            ...
                                            <input class="count count-input" value="{{ $val['quantity'] }}"
                                                data-price="{{ $val['price']+$product['damage_wavier_tax'] }}"
                                                data-cart_id="{{ $val['cart_id'] }}">
                ...
                <div class="tf-cart-footer-inner">
                    <div class="tf-free-shipping-bar">
                        ...
                        <div class="tf-progress-msg">
                            Buy <span class="price fw-6">PKR 2999</span> more to enjoy <span class="fw-6">Free
                                Shipping</span>
                        </div>
                    </div>
                    <div class="tf-page-cart-checkout">
                        ...
                        <div class="tf-cart-totals-discounts">
                            <h3>Subtotal</h3>
                            <span class="total-value">PKR {{ isset($total) ? $total : 0 }}</span>
                        </div>
                        <div class="tf-cart-totals-discounts">
                            <h3>Shipping</h3>
                            @php
                            ...
                            @endphp
                            {{-- <span class="total-value">PKR {{$config['SHIPPINGPRICE']}}</span> --}}
                            <span class="total-value">
                                {{ $total < 2999 ? 'PKR ' . $config['SHIPPINGPRICE'] : 'Free Delivery' }}
                            </span>
                        ...
                        <div class="tf-cart-totals-discounts">
                            <h3>Total</h3>
                            @if($total < 2999)
                                <span class="total-value">PKR {{ isset($total) ? $total + $config['SHIPPINGPRICE'] : 0 }}</span>
                                @else
                                <span class="total-value">PKR {{ $total }}</span>
                                @endif
                        </div>
...
<script type="text/javascript">
    ...
    var $countInput = $(this).closest('.cart-quantity-inputs').find(".count-input");
    var qty = parseInt($countInput.val());
    var rowid = $countInput.data('cart_id');
    var price = parseFloat($countInput.data('price'));
    ...
    $countInput.val(qty); // update input
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status == 1) {
                ...
                $(`tr[data-rowid="${rowid}"] .cart-total`).text('PKR ' + res.item_sub_total);
                // Update cart grand total
                $('#cart-grand-total').text('PKR ' + res.cart_total);
