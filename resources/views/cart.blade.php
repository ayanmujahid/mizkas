                                ...
                                <tr class="tf-cart-item file-delete" data-rowid="{{ $val['cart_id'] }}">
                                    ...
                                    <td class="tf-cart-item_product">
                                        ...
                                        <div class="cart-info">
                                            ...
                                            <a href="{{ route('remove-cart', $val['cart_id']) }}" class="cart-delete"
                    ...
                    <div class="tf-cart-footer-inner">
                        ...
                        <div class="tf-page-cart-checkout">
                            ...
                            <div class="tf-cart-totals-discounts">
                                <h3>Subtotal</h3>
                                <span class="total-value" id="cart-subtotal">
                                    PKR {{ $total ?? 0 }}
                                </span>
                            </div>
                            <div class="tf-cart-totals-discounts">
                            ...
                            <div class="tf-cart-totals-discounts">
                                ...
                                <span class="total-value" id="cart-grand-total">
                                    PKR {{ $total < 5000 ? $total + $config['SHIPPINGPRICE'] : $total }}
                                </span>
                            </div>
...
@section('js')
    ...
    <script>
        $(document).on('click', '.changeqty', function() {
            ...
            let input = $(this).closest('.wg-quantity').find('.count-input');
            let qty = parseInt(input.val());
            let rowid = input.data('cart_id');
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status === 1) {
                        ...
                        // Item total
                        $('tr[data-rowid="' + rowid + '"] .cart-total')
                            .text('PKR ' + res.item_sub_total);

                        // Subtotal
                        $('#cart-subtotal').text('PKR ' + res.subtotal);

                        // Shipping price
                        if (res.shipping > 0) {
                            $('#cart-shipping').text('PKR ' + res.shipping);
                        } else {
                            $('#cart-shipping').text('Free Delivery');
                        }

                        // Grand total
                        $('#cart-grand-total').text('PKR ' + res.grand_total);

                        // 🔥 Free shipping message (THIS FIXES IMAGE #3)
                        if (res.remaining > 0) {
                            $('#free-shipping-text')
                                .html('Buy <strong>PKR ' + res.remaining +
                                    '</strong> more to enjoy <strong>Free Shipping</strong>');
                        } else {
                            $('#free-shipping-text')
                                .html('<strong>🎉 You have unlocked Free Shipping!</strong>');
                        }
                    }
                }
